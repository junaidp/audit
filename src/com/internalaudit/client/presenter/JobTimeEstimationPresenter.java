...
//				display.getEstimatedWeeks().setText(String.valueOf(result.getJobTimeEstimation().getEstimatedWeeks()));
				...
				if ( result != null )				{
					...
					{
						if ( display.getEstWeeksListBox().getItemText(i).equals( String.valueOf( result.getJobTimeEstimation().getEstimatedWeeks())))
						{
					...
					display.getJobTimeEstId().setText( String.valueOf( result.getJobTimeEstimation(). getJobTimeEstimationId()));
					
					display.getEstWeeksListBox().addItem( String.valueOf(result.getJobTimeEstimation().getEstimatedWeeks()) );
					
					//display.getFieldWorkManHours().setText( String.valueOf(result.getJobTimeEstimation().getManagementHours() ));
					
					display.getTotalWorkingManHours().setText(String.valueOf( result.getJobTimeEstimation().getHoursInclTravel() ));
					
					display.getFieldWorkManHours().setText( String.valueOf(result.getJobTimeEstimation().getFieldWorkManHours()) );
					
					display.getMgmtHours().setText(String.valueOf( result.getJobTimeEstimation().getManagementHours() ));
					
					display.getHoursInclusiveOfTravel().setText( String.valueOf(result.getJobTimeEstimation().getHoursInclTravel()) );
					
					for (int i = 0; i< display.getPlaceofWorkListBox().getItemCount(); i++)
					{
						if ( display.getPlaceofWorkListBox().getItemText(i).equals( String.valueOf( result.getJobTimeEstimation().getTravelDays() ) ) )
						{	
					...
					//display.getPlaceofWorkListBox().addItem( result.getJobTimeEstimation().getPlaceOfWork());
					...
					{
						if ( display.getTravelingDaysListBox().getItemText(i).equals( String.valueOf( result.getJobTimeEstimation().getTravelDays() ) ) )
							display.getTravelingDaysListBox().setSelectedIndex(i);
					}
					
					display.getTotalWorkingManHoursTextBox().setText(String.valueOf( result.getJobTimeEstimation().getTotalWorkingManHours()) );	
		...
		display.getHeading().setText( display.getStrategicDTO().getStrategicObjective() );
		...
		display.getSaveJobTimeEst().addClickHandler( new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				try {
					...
					jobTimeEstEntity.setEstimatedWeeks( Integer.parseInt(display.getEstWeeksListBox().getItemText( display.getEstWeeksListBox().getSelectedIndex())));
					
					jobTimeEstEntity.setFieldWorkManHours( Integer.parseInt(display.getFieldWorkManHours().getText().toString()) );
					
					jobTimeEstEntity.setTotalWorkingManHours (Integer.parseInt(display.getTotalWorkingManHours().getText().toString()));
					
					jobTimeEstEntity.setManagementHours( Integer.parseInt(display.getMgmtHours().getText().toString() ) );
					
					jobTimeEstEntity.setHoursInclTravel( Integer.parseInt( display.getHoursInclusiveOfTravel().getText().toString() ));
					
					jobTimeEstEntity.setPlaceOfWork( display.getPlaceofWorkListBox().getItemText( display.getPlaceofWorkListBox().getSelectedIndex()) );
					
					jobTimeEstEntity.setScopeOfWork ( display.getHighLevelScopeOfWork().getText().toString() );
					
					jobTimeEstEntity.setTravelDays ( Integer.parseInt( display.getTravelingDaysListBox().getItemText( display.getTravelingDaysListBox().getSelectedIndex()) ));
					
					jobTimeEstEntity.setHoursInclTravel( jobTimeEstEntity.getTotalWorkingManHours() + jobTimeEstEntity.getTravelDays() * 8);
					
					//since the size of listbox is dynamic, we need to get it at the runtime
					//0th widget of the VerticalPanel is always a 'SkillResource' 
					...
					//loop through all the list boxes and get the selected index and value / text
					...
					{		
							...
							int hoursReqd = Integer.parseInt(res.getNoOfResources().getText().toString()) * 40 * jobTimeEstEntity.getEstimatedWeeks();
							
							if ( hoursReqd > res.getAvailableHours() )
							{
								maxHoursReached = false;
							
								System.out.println("You have selected more hours than available hours for " + res.getSkillsList().getItemText(res.getSkillsList().getSelectedIndex()));
		...
		display.getPlaceofWorkListBox().addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				if ( place.equals("Inhouse")) {
				   ...
				   //if the user previously selected outstation, this will recalculate the traveling hours and
				   // hours incl travel
					
					int hoursToSubt = 
							Integer.parseInt( 
									display.getTravelingDaysListBox().getItemText( 
											display.getTravelingDaysListBox().getSelectedIndex()
									) 
							);
					
					int currentHoursForTravel = Integer.parseInt(display.getHoursInclusiveOfTravel().getText());
					
					display.getHoursInclusiveOfTravel().setText( String.valueOf( currentHoursForTravel - hoursToSubt * 8 ));
	...
	private void fetchSkillResources(final ArrayList<ResourceUse> resourceList,final boolean updateState) {
		rpcService.fetchSkills(new AsyncCallback<ArrayList<Skills>>() {
			...
			public void onSuccess(ArrayList<Skills> skillsList) {	
				...
				{
					SkillsResources skillsResources = new SkillsResources();
					
						skillsResources.getSkillsList().addItem(skillsList.get(i).getSkillName(), skillsList.get(i).getSkillId()+"");	
						
						skillsResources.setAvailableHours( skillsList.get(i).getAvailableHours());
				...
				if ( updateState && resourceList != null)
				{
					for( int i = 0; i < resourceList.size(); i++)
					{
						...
						((SkillsResources)display.getSkillsResourceContainer().getWidget(i)).getSkillsList().addItem(resourceList.get(i).getSkill().getSkillName(), resourceList.get(i).getSkill().getSkillId()+"");	

						((SkillsResources)display.getSkillsResourceContainer().getWidget(i)).getNoOfResources().setText( String.valueOf(resourceList.get(i).getNoOfResources()) );
													
						((SkillsResources)display.getSkillsResourceContainer().getWidget(i)).getSkillsList().setSelectedIndex(i);
	...
	private void fetchCheckBoxState() {
		rpcService.fetchCheckBoxStateFor( display.getStrategicDTO().getStrategicId(), new AsyncCallback<ArrayList<JobAndAreaOfExpertise>>() {
	...
	private void calculateManHours() {
		...
		{	
			...
			if ( indexSelected == -1) display.getEstWeeksListBox().setSelectedIndex(0);
			...
			for ( int i = 0; i < listSize; ++i)
				totalResources += Integer.parseInt(((SkillsResources)display.getSkillsResourceContainer().getWidget(i)).getNoOfResources().getText());
	...
	private void calculateHoursInclTravel() {
		...
		try
		{
			...
			int travelDays = Integer.parseInt( display.getTravelingDaysListBox().getItemText( display.getTravelingDaysListBox().getSelectedIndex()) );
